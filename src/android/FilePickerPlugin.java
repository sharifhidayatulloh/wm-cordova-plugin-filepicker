...
public class FilePickerPlugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if ("selectFiles".equals(action)) {
            ...
            if (!hasReadPermission()) {
                ...
                getReadPermission(action, 0, callbackContext);
                // getWritePermission(action, 1, callbackContext);
            } else {
                this.selectFiles();
            }
        } else {
    ...
    private void getReadPermission(String rawArgs, int action, CallbackContext callbackContext) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            PermissionHelper.requestPermissions(this, action, 
            ...
            PermissionHelper.requestPermission(this, action, Manifest.permission.READ_EXTERNAL_STORAGE);
          }
    }

    private void getWritePermission(String rawArgs, int action, CallbackContext callbackContext) {
        // int requestCode = pendingRequests.createRequest(rawArgs, action, callbackContext);
        PermissionHelper.requestPermission(this, action, Manifest.permission.WRITE_EXTERNAL_STORAGE);
    }
